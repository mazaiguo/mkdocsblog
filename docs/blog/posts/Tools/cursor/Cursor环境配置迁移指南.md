...
tags:
  ...
  - ObjectARX
  - WPF
description: 记录 Cursor IDE 的完整配置，包含全局规则、项目规则、MCP 工具、Skills 技能库、CAD UI 设计规范等，换新电脑时可直接参照还原
author: JerryMa
---

# Cursor 开发环境配置迁移指南

本文档合并了全局配置、项目规则（.cursorrules）、CAD UI 设计规范的完整内容，换新电脑时按步骤操作即可还原。
...
| **cursor.rules** | settings.json | 技术规范：编码、Vue、C++、C#、UI、Git |
| **.cursorrules** | 项目根目录 | 项目细节：完整 UI 规范、组件模板、布局、验证 |
...
- 登录账号（Settings > Sign in）

---

## 二、User Rules（最高优先级规则）

`Ctrl+Shift+J` → **General** → **User Rules**，添加以下规则（每行一条）：
...
## 三、Rules for AI（全局技术规范）

### 设置方式 A：通过 UI 设置（推荐）

`Ctrl+Shift+J` 打开 Cursor Settings → 找到 **Rules for AI** 文本框 → 粘贴以下内容：
...
- Excel：使用 `EPPlus`（ExcelPackage）或 `ClosedXML`；优先 EPPlus（5+ 商用需授权，ClosedXML 为 MIT）
- CSV：使用 `CsvHelper`（CsvReader/CsvWriter）
- HTTP 请求：使用 `HttpClient`（推荐）或 `RestSharp`
- 日志：使用 `Serilog`（结构化日志，支持多种 Sink）
...
## 四、项目级规则（.cursorrules 完整内容）

将以下内容保存为项目根目录的 `.cursorrules` 文件，Clone 后自动生效。

> 该文件包含完整的 CAD WebView UI 开发规则和设计规范，是 Rules for AI 的详细版本。
...
.splitter-handle { width: 8px; background: #f0f0f0; border-right: 1px solid var(--cad-border); cursor: col-resize; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
...
| Excel | `libxl` 或 `OpenXLSX` | 读写 xls/xlsx |
...
| 用途 | 推荐库 (NuGet) | 说明 |
|------|----------------|------|
| JSON | `Newtonsoft.Json` 或 `System.Text.Json` | 优先 Newtonsoft，简单场景可用内置 |
...
| HTTP | `HttpClient`（推荐）或 `RestSharp` | async/await 友好 |
| DI | `Microsoft.Extensions.DependencyInjection` | 依赖注入容器 |
...
- 使用 Vite 构建，输出单文件 HTML 内联所有资源
- 禁止引用任何外部 CDN 资源（字体、脚本、样式表等）
- 所有图标使用 Element Plus 内置图标或 SVG 内联
- 不使用 Google Fonts 等远程字体，使用系统字体栈
...
- [ ] 无任何外部 CDN 引用（link/script/font-face）
- [ ] 所有图片资源使用 Base64 内联或 SVG
- [ ] `npm run build` 后 dist/ 目录只有一个 index.html
- [ ] 双击打开 index.html 功能正常（无需 HTTP 服务器）
- [ ] WebView2 加载本地文件路径可正常运行
...
> **使用方法**：直接复制上方 ```` ````markdown ... ```` ```` 内的全部内容，保存为项目根目录的 `.cursorrules` 文件。
...
**方法 2：从本文档第九节复制内容（无需 Git）**

- 将 `industrial-ui-pro` 的内容保存为 `C:\Users\<用户名>\.cursor\skills-cursor\industrial-ui-pro\SKILL.md`
- 将 `vue-prototype-builder` 的内容保存为 `C:\Users\<用户名>\.cursor\skills-cursor\vue-prototype-builder\SKILL.md`

> Cursor 内置的 5 个 Skills（create-rule、create-skill 等）会随 Cursor 自动安装，无需手动复制。
...
- 安装 [Python](https://www.python.org/) 并确保 `pip` 可用
- 安装 `uv`（用于运行 `uvx`）：
  ```bash
  pip install uv
  ```
- 安装 Node.js（用于 `npx`）：[https://nodejs.org/](https://nodejs.org/)
- 将上方 mcp.json 内容保存到 `C:\Users\<用户名>\.cursor\mcp.json`
- 重启 Cursor，在 Cursor Settings > MCP 中验证工具已加载
...
- [ ] 安装 Cursor 并登录账号
- [ ] 设置 User Rules（第二步）
- [ ] 设置 Rules for AI（第三步，方式 A 或 B）
- [ ] 复制 Skills 到全局目录（第五步）
- [ ] 安装并配置 MCP 工具（第六步）
- [ ] 配置编辑器设置（第七步）
- [ ] 安装推荐扩展（第八步）
- [ ] Clone 项目代码（`.cursorrules` 自动生效）
- [ ] 重启 Cursor，验证 MCP 工具已加载

---

## 九、Skill 完整内容（用于新电脑还原）
...
## 模板 1：SplitPanel 分栏布局（支持拖拽调整宽度）

```vue
<template>
  <div class="cad-split-panel" :class="{ 'is-dragging': isDragging }">
    ...
    <div class="splitter"
      ...
      @dblclick="sidebarOpen = !sidebarOpen">
...
const onSplitterMouseDown = (e: MouseEvent) => {
  if (!sidebarOpen.value) { sidebarOpen.value = true; return }
...
.cad-split-panel.is-dragging { cursor: col-resize; user-select: none; }
...
.splitter { width: 8px; background: #f0f0f0; border-right: 1px solid #d4d4d4; cursor: col-resize; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
...
> 文档更新时间：2026-03-19
> 合并自：Cursor环境配置迁移指南.md + cad-webview-ui-rules.md + .cursorrules
